...
{
	ThirdVideoEnc::ThirdVideoEnc(string ifname, string ofname, char mode, uint predictMode, uint blockSize, uint searchArea, uint periodicity, char thresholdMode, double t, double base_threshold, double ceil_threshold, double min_threshold, char searchMode)
	{
		this->mode = mode;
		switch(mode)
		{
	...
	ThirdVideoEnc::~ThirdVideoEnc()
	{
		save();
	}
	
	void ThirdVideoEnc::writeHeader(Mat* frame, uint fps)
	{
		if (golomb==NULL)
		{
	...
	double ThirdVideoEnc::calcThreshold(Mat a, Mat b)
	{
		Mat diff;
		Scalar sdiff;
		Mat diffChannels[3];
	...
	Point ThirdVideoEnc::search(Mat s, Mat tpl, int x, int y, double* maxVal)
	{
		*maxVal = 1000.0;
		double aux;
		Point maxLoc;
	...
	Mat ThirdVideoEnc::encode(Mat* frame, uint fps)
	{
	...
	void ThirdVideoEnc::encodeInter(Mat* aBlock, Mat* lBlock)
	{
		uchar aB, aG, aR;
		uchar lB, lG, lR;
	...
	void ThirdVideoEnc::encodeIntra(Mat* frame)
	{
	...
	bool ThirdVideoEnc::decode(Mat* frame)
	{
	...
	bool ThirdVideoEnc::decodeIntra(Mat* frame)
	{
	...
	bool ThirdVideoEnc::decodeInter(Mat* aBlock, Mat* lBlock)
	{
